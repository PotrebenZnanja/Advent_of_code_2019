...
void splitStrToVec(std::vector<std::string>& vek, const std::string& text)
{
	...
	for (int l = 0; l < temp.size(); l++)
		paste += temp[l];
	vek.push_back(paste);
	/*for (std::size_t i = 0; i < vek.size(); i++)
		std::cout << vek[i]<<",";*/
}

int premikSt(std::string& ukaz)
{
	int m = 0;
	for (int i = 1; i < ukaz.size(); i++)
	{
		//std::cout << (int)(ukaz.at(i)-'0')<<","<<ukaz.size()<< std::endl;
		m += (int)(ukaz.at(i) - '0') * (int)std::pow(10, (ukaz.size() - i - 1));
		//std::cout <<ukaz << " se premakne za "<< premik << std::endl;
	}
	return m;
}

std::pair<int,int> nastavi_pot(char**& map, std::string& ukaz,int vrstica,int stolpec,bool vec)
{
	//return x and y in vector
	int premik=premikSt(ukaz);
	char poti[2];
	if (vec)
	{
		poti[0]= '|';
	...
	std::cout << ukaz << " se premakne za " << premik << std::endl;
	std::cout << sizeof(map[1]) << "  " << sizeof(map)<<std::endl;
	switch (ukaz.at(0))
	{
	case 'U':
		std::cout << stolpec << " - " << premik << std::endl;
		for (int i = 1; i < premik; i++)
		{
			if (map[vrstica - i][stolpec] != '.' && map[vrstica - i][stolpec] != 'o')
				map[vrstica - i][stolpec] = 'X';
			else
				map[vrstica - i][stolpec] = poti[0];
		}

		if (map[vrstica - premik][stolpec] != '.' && map[vrstica - premik][stolpec] != 'o')
			map[vrstica - premik][stolpec] = 'X';
		else
			map[vrstica - premik][stolpec] = '+';
		vrstica -= premik;
		break;
	...
	case 'L':
		for (int i = 1; i < premik; i++)
		{
			if (map[vrstica][stolpec - i] != '.' && map[vrstica][stolpec - i] != 'o')
				map[vrstica][stolpec - i] = 'X';
			else
				map[vrstica][stolpec-i] = poti[1];
		}
		if (map[vrstica][stolpec-premik] != '.' && map[vrstica][stolpec-premik] != 'o')
			map[vrstica][stolpec-premik] = 'X';
		else
			map[vrstica][stolpec-premik] = '+';
		stolpec -= premik;
		break;
	...
	return std::make_pair(vrstica, stolpec);
}
...
int main()
{
	int vrstice = 20400;
	int stolpci = 20401;
	char** mapWire = (char**)malloc(vrstice * sizeof(char*));
	for (int i = 0; i < vrstice; i++)
	{
		mapWire[i] = (char*)malloc(stolpci * sizeof(char));
		for (int j = 0; j < stolpci; j++)
		{
			if (i == j && i == vrstice / 2 - 1)
				mapWire[i][j] = 'o';
			else
				mapWire[i][j] = '.';
			if (j == stolpci - 1)
	...
	if (txtFile.is_open())
	{
		...
		txtFile.close();
		std::cout << "Minimal distance: " << min << std::endl;
	}
}